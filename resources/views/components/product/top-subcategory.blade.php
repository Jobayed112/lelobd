<div class="py-8 bg-gray-50">
    <div class="container mx-auto bg-white rounded-lg shadow-lg p-4 sm:p-6">
        <h1 class="text-2xl sm:text-3xl font-semibold mb-4 text-center text-indigo-700">Top Sub Categories</h1>
            ...
            @foreach ($categories as $category)  <!-- Loop through categories -->
                @foreach ($category->subcategories as $subcategory)  <!-- Loop through subcategories of each category -->
                    <div class="text-center ring-1 hover:ring-4 bg-white rounded-xl shadow-md hover:shadow-2xl transition-all duration-300">
                        ...
                        <p class="mt-2 text-sm sm:text-lg font-medium text-gray-800">{{ $subcategory->name }}</p>
                    </div>
                @endforeach
            @endforeach
        </div>
